...
// Android project's `tauri.settings.gradle`, which is what actually matters
// here: Kotlin-side native code, invoked only via `run_mobile_plugin` calls
// this crate's own Rust API makes internally.
const COMMANDS: &[&str] = &[];
